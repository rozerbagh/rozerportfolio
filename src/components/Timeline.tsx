export default function Timeline() {
  return (
    <div className="container">
      <ol className="timeline">
        <li>
          <p className="line">Experiences</p>
          <span className="point"></span>
          <p className="description">Fullstack Developer @Zybisys</p>
          <span className="date"><strong>Sep. 2020 - Feb. 2022</strong></span>
        </li>
        <li>
          <p className="line">Experiences</p>
          <span className="point"></span>
          <p className="description">Software Engineer @Zerozilla</p>
          <span className="date"><strong>Mar. 2022 - Today</strong></span>
        </li>

        <li>
          <p className="line">Experiences</p>
          <span className="point"></span>
          <p className="description">Freelance</p>
          <span className="date"><strong>Jan. 2019 - Today</strong></span>
        </li>

        <li>
          <p className="line">Education</p>
          <span className="point"></span>
          <p className="description">Nitte Meenakshi Institute of Technology</p>
          <span className="date"><strong>2014 - 2018</strong></span>
        </li>
      </ol>
    </div>
  )
}
